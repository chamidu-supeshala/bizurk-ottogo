...
(function (window, $, undefined) {
  ...
  function profilePicture(cssSelector, imageFilePath, options) {
    ...
    function init(cssSelector, imageFilePath, options) {
       ...
       * Start canvas
       */
      self.canvas.width = self.photoFrame.outerWidth() * pxRatio;
      self.canvas.height = self.photoFrame.outerHeight() * pxRatio;
      self.canvas.style.width = self.photoFrame.outerWidth() + "px";
      self.canvas.style.height = self.photoFrame.outerHeight() + "px";
      ...
      registerDropZoneEvents(options);
    ...
    function removeImage() {
      self.canvasContext.clearRect(
        0,
        0,
        self.model.cropWidth * pxRatio,
        self.model.cropHeight * pxRatio
      );
      ...
      self.imageHelperCanvasContext.clearRect(
        0,
        0,
        self.imageHelperCanvas.width * pxRatio,
        self.imageHelperCanvas.height * pxRatio
      );
     ...
     */
    function registerDropZoneEvents(options) {
      var target = null;
      /**
    ...
    function render() {
      ...
      self.canvasContext.drawImage(
        self.model.imageSrc,
        self.model.x * pxRatio,
        self.model.y * pxRatio,
        self.model.width * pxRatio,
        self.model.height * pxRatio
      );
    ...
    function registerImageHelper() {
      ...
      canvas.style.width = self.photoHelper.outerWidth() + "px";
      canvas.style.height = self.photoHelper.outerHeight() + "px";

      canvas.width = self.photoHelper.outerWidth() * pxRatio;
      canvas.height = self.photoHelper.outerHeight() * pxRatio;
